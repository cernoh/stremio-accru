...
-- Accru patch: graceful degrade when shaders/VPY missing (empty shaders dir -> no-op instead of mpv "not found")
local function file_exists(path)
    local expanded = mp.command_native({"expand-path", path})
    local info = utils.file_info(expanded)
    if info then return true end
    info = utils.file_info(path)
    return info ~= nil
end
local function filter_existing_shaders(chain)
    if not chain or chain == "" then return "" end
    local out = {}
    for token in string.gmatch(chain, "[^;]+") do
        token = token:match("^%s*(.-)%s*$")
        if token ~= "" then
            if file_exists(token) then table.insert(out, token) else log("Shader missing, skipping: " .. token) end
        end
    end
...
local function apply_anime_shaders(preset, is_legacy, is_hdr)
    ...
    -- Accru: filter to only existing files so empty/missing shaders dir degrades cleanly
    shader_chain = filter_existing_shaders(shader_chain)
    if shader_chain == "" then
        log("No shaders found on disk, skipping glsl-shaders (shaders/README.md)")
...
local function apply_anime_vf(is_legacy, svp_enabled)
    ...
    if svp_enabled then
        local svp_path = VF_FILTERS.svp_anime:match('vapoursynth="([^"]+)"')
        if svp_path and not file_exists(svp_path) then
            log("SVP VPY missing, skipping SVP (desktop opt-in, see svp_anime.vpy): " .. svp_path)
            return
        end
...
function try_execute_profile()
    ...
    elseif should_run_global_svp then
        local svp_cinema_path = VF_FILTERS.svp_cinema:match('vapoursynth="([^"]+)"')
        if svp_cinema_path and not file_exists(svp_cinema_path) then
            log("SVP Cinema VPY missing, skipping global SVP: " .. svp_cinema_path)
        else
